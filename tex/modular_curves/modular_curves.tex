...
\begin{proposition}
The maps $h_A$ and $h^A$ from \Cref{def:hom-functor} are a covariant and a contravariant funtor respectively. 
\end{proposition}
\begin{proof}
...
\hspace{1em}\todo[inline]{cite A. Enge. Elliptic curves and their applications to cryptography — an introduction. Kluwer, 199 and Jinbi Jin \url{https://arxiv.org/pdf/1303.4327}}

Let $\mathcal W := y^2+ a_1xy+a_3y-(x^3+a_2x^2+a_4x+a_6) \in \Z[a_1,a_2,a_3,a_4,a_6,x,y]$ be the inhomogenous version of the equation defining the Weirestrass Model from  \Cref{def:weierstrass-model-over-K} obtained by setting $z=1$.
...
\end{align*}
where the $b_i$ are as in \Cref{def:weierstrass-discriminant}.
\end{definition}
...
\end{proposition}
\begin{proof}
\todo[inline]{cite enge/jinbi jin}
\end{proof}
...
\begin{definition}\label{def:representable-moduli-problem}
	A moduli problem $\mathcal P$ on $\Ell_K$ is called {\it representable} if it is representable as a contravariant functor in the sense of \Cref{def:representable-functor}.
\end{definition}

In other words $\mathcal P$ is representable  if and only if  there exists and object $E_{\mathcal P}/Y_{\mathcal P} \in \Ell_K$ and a natural isomorphism of functors $\eta: h^{E_{\mathcal P}/Y_{\mathcal P}} \to \mathcal P$. As in \Cref{def:universal-pair} we can define $t_{\mathcal P} := \eta(\id_{E_{\mathcal P}/Y_{\mathcal P}}) \in \mathcal P (E_{\mathcal P}/Y_{\mathcal P}) $. Recall from  \Cref{def:universal-pair} that $(E_{\mathcal P}/Y_{\mathcal P},t_{\mathcal P})$ is called the universal pair representing $\mathcal P$. In the context of moduli problems there are at least two different terminologies that are used to refer to this universal pair.
